...
pub struct Deserializer<'de> {
    ...
    pub(crate) handles: &'de [u8],
}

impl<'de> Deserializer<'de> {
    /// Obtain a Deserializer from a slice of bytes
    pub fn from_bytes(input: &'de [u8], handles: &'de [u8]) -> Self {
        Self { input, handles }
    }
}

impl<'de> Deserializer<'de> {
    fn try_take_handle(&mut self) -> Result<&'de [u8]> {
        if self.handles.len() >= 8 {
            let (a, b) = self.handles.split_at(8);
            self.handles = b;
            Ok(a)
        } else {
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
        ...
        V: Visitor<'de>,
    {
        if name == "Channel" {
            let mut buf = [0u8; 8];
            buf[..].copy_from_slice(self.try_take_handle()?);
            visitor.visit_u64(u64::from_le_bytes(buf))
        }
        else {
